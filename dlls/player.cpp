...
#define	FLASH_CHARGE_TIME	 0.3 // 100 units/20 seconds  (seconds per unit)
...
static void ThrowGib( entvars_t *pev, char *szGibModel, float flDamage )
{
	...
	pevNew->avelocity.x = RANDOM_FLOAT( 0, 800 );
	pevNew->avelocity.y = RANDOM_FLOAT( 0, 800 );
	pevNew->avelocity.z = RANDOM_FLOAT( 0, 800 );
...
static void ThrowHead( entvars_t *pev, char *szGibModel, floatflDamage )
{
	...
	pev->avelocity = RANDOM_FLOAT( -1, 1 ) * Vector( 0, 800, 0 );
	pev->origin.z -= 24;
